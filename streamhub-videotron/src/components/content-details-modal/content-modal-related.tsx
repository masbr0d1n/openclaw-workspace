 ...
 * Displays related videos grid with 4 items
 */
...
export function ContentModalRelated({
  ...
  useEffect(() => {
    // Filter related videos based on channel
  ...
  // Get category - using 'Video' as default since Video type doesn't have tags
  const getCategory = (_video: Video): string => {
    return 'Video';
  };
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {relatedVideos.map((relatedVideo) => (
          <div
            ...
            className="related-card bg-white rounded-lg overflow-hidden shadow-md cursor-pointer border border-gray-200 hover:shadow-lg hover:scale-102 transition-all"
          >
            ...
            <div className="p-3">
              ...
              <div className="flex items-center justify-between text-xs text-gray-600">
                ...
                <Badge className="bg-info text-white px-2 py-0.5 rounded-full text-[10px]">
                  {getCategory(relatedVideo)}
                </Badge>
              </div>
            </div>
          </div>
